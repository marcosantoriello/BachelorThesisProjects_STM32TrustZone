...
int init_rng() {
	...
	if (to_return != 0) {
		send_message("There was en error in initializing RNG\r\n");
		return to_return;
	} else {
		send_message("RNG successfully initialized\r\n");
	}
	return to_return;
}
...
int main(void)
{
  ...
  MX_RNG_Init();
  /* USER CODE BEGIN 2 */
  send_message("Welcome to RsaEncryption_Example!\r\n");

  /* INITIALIZING RNG */
  send_message("Initializing RNG...\r\n");
  ret = init_rng();
  if (ret != 0) {
	  return ret;
  }
